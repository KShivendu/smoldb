    ...
    for (i, rx) in rx_vec.into_iter().enumerate() {
        ...
        let handle = thread::spawn(move || loop {
            ...
            let raft_group = match node.raft_group {
                ...
                _ => {
                    // println!("Node {i} is not initialized, skipping tick.");
                    continue;
                }
            };
...
fn on_ready(
    ...
    if !ready.messages().is_empty() {
        ...
        println!("Found normal {} messages to send", ready.messages().len());
    ...
    let mut handle_committed_entries =
        |rn: &mut RawNode<MemStorage>, committed_entries: Vec<Entry>| {
            for entry in committed_entries {
                println!("Found committed entry {entry:?}");
    ...
    if !ready.persisted_messages().is_empty() {
        ...
        // I expect this to be send from followers to the leader.
